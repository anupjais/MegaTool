  ...
  try {
    ...
    for (const [index, file] of validFiles.entries()) {
      ...
      try {
        ...
        console.log(
          `${debugTag} ${fileTag} Dimensions: ${metadata.width}x${metadata.height}`
        );
    ...
    console.log(
      `${debugTag} Conversion successful: ${resultData.metrics.fileCount} files`
    );
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, resultData, "Conversion successful"));
  } catch (error) {
    // Error cleanup
    await Promise.all(
      tempFiles.map(async (path) => {
    ...
    for (const range of parsedRanges) {
      try {
        ...
        if (
          ...
          console.error(
            `[splitPdf] Invalid range ${range} for ${totalPages}-page PDF`
          );
          throw new ApiError(
            400,
            `Invalid range: ${range}. Document has ${totalPages} pages.`
          );
        }
        ...
        throw new ApiError(
          400,
          `Error processing ${range}: ${rangeError.message}`
        );
      }
    }
...
const mergePdfs = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const [index, file] of validFiles.entries()) {
      try {
        console.log(
          `[mergePdfs] Processing PDF ${index + 1}: ${file.originalname}`
        );
        ...
        console.log(
          `[mergePdfs] Added ${pages.length} pages from ${file.originalname}`
        );
      } catch (mergeError) {
        console.error(
          `[mergePdfs] Error processing ${file.originalname}:`,
          mergeError
        );
        throw new ApiError(
          500,
          `Failed to process ${file.originalname}: ${mergeError.message}`
        );
      }
    }
    ...
    console.log(
      `[mergePdfs] Successfully merged ${validFiles.length} PDFs into ${totalPages} pages`
    );
...
const removePagesFromPdf = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (pagesToRemove.length === 0) {
      ...
      throw new ApiError(
        400,
        "Invalid pages format. Use comma-separated numbers"
      );
    }
    ...
    if (invalidPages.length > 0) {
      ...
      throw new ApiError(
        400,
        `Invalid pages: ${invalidPages.join(", ")}. Document has ${totalPages} pages.`
      );
    }
    ...
    console.log(
      `${debugTag} Sending response with ${base64Pdf.length} chars base64 data`
    );
...
const protectPdf = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!req.body.password || req.body.password.length < 8) {
      console.error(
        `${debugTag} Invalid password length: ${req.body.password?.length || 0}`
      );
    ...
    console.debug(
      `${debugTag} Generated owner password: ${ownerPassword.substring(0, 6)}...`
    );
    ...
    const statusCode = error.statusCode || 500;
    const message =
      error instanceof ApiError ? error.message : "PDF protection failed";
    throw new ApiError(statusCode, message);
  }
});
