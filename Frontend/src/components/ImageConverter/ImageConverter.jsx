...
function ImageConverter() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const API_URL =
        import.meta.env.VITE_API_BASE_URL || import.meta.env.VITE_LOCAL_API;
  ...
  return (
    <div className=" flex flex-col items-center bg-gray-100 p-4 md:p-5  ">
      ...
      >
        ...
        <div
          ref={scrollRef}
          className="flex gap-2 overflow-x-auto whitespace-nowrap w-full px-4 md:px-8"
        >
          {" "}
          {/* Reduced padding */}
          {files.map((file, index) => (
            <div key={index} className="relative text-center">
              ...
              />
              <p className="text-xs mt-0.5 md:mt-1 truncate w-20 md:w-24">
                {file.name}
              </p>
              <button
