  ...
  return (
    <div>
      ...
      <nav className="bg-white text-black p-4 flex justify-between items-center shadow-2xl fixed top-0 left-0 w-full z-10">
        <div className="text-xl font-bold hidden md:block">MegaToolbox</div>
        <ul className="flex gap-6 relative">
          {/* Image Tools Dropdown */}
          <li
            className="relative"
            onMouseEnter={() => setDropdown("image")}
            onMouseLeave={() => setDropdown(null)}
          >
            <button className="hover:text-yellow-400 px-3 py-2 border rounded-md">
              Image Tools
            </button>
            {dropdown === "image" && (
      ...
      >
        ...
        <div className="mt-6 p-4 bg-white shadow-md rounded-lg">
          ...
          <p className="text-gray-600">
            Convert images from one format to another seamlessly.
          </p>
      ...
      <section id="pdf-tools" className="px-5 py-10 bg-gray-100 text-center">
